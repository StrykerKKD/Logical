...
let test_equal_on_set_variable_and_empty_state () =
  let test_set = Value.set (Base.Set.singleton (module Value.Comparator) (Value.int 1)) in
  let expected = [Some ["a",test_set]] in
  let actual = equal (Value.var "a") test_set State.empty |> Base.Sequence.to_list in
  Alcotest.(check int) "equal should return with state which contains the set variable" 
    0 (compare expected actual)

let test_equal_on_set_variable_and_matching_state () =
  let test_set = Value.set (Base.Set.singleton (module Value.Comparator) (Value.int 1)) in
  let expected = [Some ["a",test_set;"b",test_set]] in
  let actual = equal (Value.var "a") (Value.var "b") ["b",test_set] |> Base.Sequence.to_list in
  Alcotest.(check int) "equal should return with state which contains the set variable" 
    0 (compare expected actual)
...
let test_in_set_on_variables_and_empty_state () =
  let test_set = Value.set (Base.Set.of_list (module Value.Comparator) [Value.int 1; Value.int 2]) in
...
let tests = [
  ...
  "test_equal_on_set_variable_and_empty_state", `Quick, test_equal_on_set_variable_and_empty_state;
  "test_equal_on_set_variable_and_matching_state", `Quick, test_equal_on_set_variable_and_matching_state;
